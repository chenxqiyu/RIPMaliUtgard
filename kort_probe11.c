/*
 * kort_probe11.c - 修正 WB_PITCH = w*4/8 (=8) 验证连续写
 *
 * 之前 pitch=(w*4+64)/8=16 -> 128B 步进 -> 64B开/64B空 散射
 * 修正 pitch=w*4/8=8 -> 64B 步进 -> 连续
 *
 * T1: 'A' @ +0x2000, w=0x10 h=0x10 pitch=8 -> dump [0x2000,0x2500)
 *     预期: 0x400 (1KB) 全 A 连续
 * T2: 细粒度 'B'@+0x2000 再 'C'@+0x2004 -> dump 看是否 BBBBCCCC 连续
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    printf("[*] kort_probe11 - corrected pitch=8 contiguity test\n");
    ...
    /* T1: pitch=8, w=0x10 h=0x10 -> 预期 0x400 连续 A */
    memset((char*)buf + 0x2000, 0, 0x600);
    __builtin___clear_cache((char*)buf, (char*)buf + BUF_SIZE);
    printf("\n[T1] 'A' @ +0x2000, w=0x10 h=0x10 pitch=8\n");
    wb_write(fd, (uint32_t*)buf, GPU_VA_DATA + 0x2000, 0x41414141, 0x10, 0x10, 8);
    dump_region("  [0x2000,0x2500)", (volatile uint8_t*)buf + 0x2000, 0x500);

    /* T2: 细粒度 'B'@+0x2000 'C'@+0x2004, pitch=8 h=0x4 */
    memset((char*)buf + 0x3000, 0, 0x100);
    __builtin___clear_cache((char*)buf, (char*)buf + BUF_SIZE);
    printf("\n[T2] 'B'@+0x3000 then 'C'@+0x3004, w=0x10 h=0x4 pitch=8\n");
    wb_write(fd, (uint32_t*)buf, GPU_VA_DATA + 0x3000, 0x42424242, 0x10, 0x4, 8);
    wb_write(fd, (uint32_t*)buf, GPU_VA_DATA + 0x3004, 0x43434343, 0x10, 0x4, 8);
    dump_region("  [0x3000,0x3080)", (volatile uint8_t*)buf + 0x3000, 0x80);
