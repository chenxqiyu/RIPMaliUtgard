/*
 * kort_pp_a8_test.c - Test A8/R8 pixel format alignment and write size
 *
 * Goal: find a format with 1-byte alignment so we can write strings byte by byte.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
 * Generic WB write with configurable pixel format.
 * clear_color is ARGB format: A=bits31-24, R=bits23-16, G=bits15-8, B=bits7-0
 */
static int wb_write_fmt(uint8_t *buf, uint32_t tgt_offset, uint32_t clear_color, uint32_t pixfmt)
{
    memset(buf, 0, 0x1000);
...
static void test_format(uint8_t *buf, const char *name, uint32_t pixfmt)
{
    printf("--- Format %s (0x%02x) ---\n", name, pixfmt);

    /* Test 1: write at offset 0, check size and pattern */
    memset(buf + TEST_OFF, FILL_BYTE, 256);
    int rc = wb_write_fmt(buf, TEST_OFF, 0x44434241, pixfmt);  /* ARGB = 0x44, R=0x43, G=0x42, B=0x41 */
    if (rc != 0) {
        printf("  FAILED (rc=%d)\n\n", rc);
        return;
    }
    ...
    printf("  Size: %d bytes (offset %d - %d)\n", mod_count, first_mod, last_mod);
    printf("  First 16: ");
    for (int i = 0; i < 16; i++) printf("%02x ", buf[TEST_OFF + i]);
    printf("\n");

    /* Test 2: alignment - write at offset 1, check if byte 0 changed */
    memset(buf + TEST_OFF, FILL_BYTE, 256);
    wb_write_fmt(buf, TEST_OFF + 1, 0x44434241, pixfmt);

    if (buf[TEST_OFF] != FILL_BYTE) {
        printf("  Alignment: >= 2 bytes (byte[0] changed)\n");
    } else {
        printf("  Alignment: 1 byte (byte[0] unchanged)\n");
    }

    /* Test 3: alignment at offset 7 */
    memset(buf + TEST_OFF, FILL_BYTE, 256);
    wb_write_fmt(buf, TEST_OFF + 7, 0x44434241, pixfmt);

    if (buf[TEST_OFF] != FILL_BYTE) {
        printf("  Alignment at off 7: byte[0] changed (>= 8 byte align)\n");
    } else {
        printf("  Alignment at off 7: byte[0] unchanged (< 8 byte align)\n");
    }

    /* Check byte pattern at offset 7 */
    printf("  Offset 7-14: ");
    for (int i = 7; i < 15; i++) printf("%02x ", buf[TEST_OFF + i]);
    printf("\n\n");
}
...
int main(void)
{
    ...
    printf("=== Format Alignment Test (1x1 frame) ===\n\n");
    ...
    test_format(buf, "RGBA8888", 0x03);  /* baseline */
    test_format(buf, "RGB565",   0x02);
    test_format(buf, "fmt0x04",  0x04);  /* R8? */
    test_format(buf, "fmt0x0B",  0x0B);
    test_format(buf, "fmt0x0C",  0x0C);
    test_format(buf, "fmt0x19",  0x19);
